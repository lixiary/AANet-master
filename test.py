...
from Ablation.net4_4 import AANet
import time
...
'''DATASETS = ['./data/SOD','./data/PASCAL-S', './data/ECSSD', './data/HKU-IS',
            './data/DUT-OMRON', './data/DUTS','./data/THUR15K']  '''
...
class Test(object):
    def __init__(self, Dataset, datapath, Network):
        ...
        self.cfg = Dataset.Config(datapath = datapath, snapshot='./ours/model-30',  mode='test')    #./ours/model-40
    ...
    def accuracy(self):
        with torch.no_grad():
            ...
            for image, mask, (H, W), maskpath in self.loader:
                ...
                #out2, out3, out4, out5 , out6 = self.net(image)
    ...
    def save(self):
        with torch.no_grad():
            for image, mask, (H, W), name in self.loader:
                #out2, out3, out4, out5, out6 = self.net(image.cuda().float())
