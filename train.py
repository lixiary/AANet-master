...
def train(Dataset, Network):
    ...
    for epoch in range(cfg.epoch):
        ...
        while image is not None:
            ...
            batch_idx += 1
            global_step += 1
            #out2, out3, out4, out5,  out6 = net(image)
           ...
           # loss6                  = F.binary_cross_entropy_with_logits(out6, mask)
            #loss                   = loss2*1 + loss3*0.8 + loss4*0.6 + loss5*0.2 +loss6*0.1   #计算Loss
            loss = loss2 * 1 + loss3 * 0.75 + loss4 * 0.25 + loss5 * 0.125
